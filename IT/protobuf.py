...
class Codec(object):
    ...
    def decode(self, msg):
        '''
            ...
            binascii用于在ASCII 和二进制之间互换,msg是一个进制数组
            funcode 消息返回的功能号
        '''
        assert_greater_equal(len(msg), 8, binascii.b2a_hex(msg))
        funcode, session_id = struct.unpack_from('<ll', msg, 0)#从msg bytes中取两个字节出来
        protobuf_data = self.data_class_dict[funcode]()#create the respone object
        protobuf_data.ParseFromString(msg[8:])#msg字节8后面的数据代表消息返回的对象
