...
import msgbus, msgbus_perf_h, protobuf_dict
...
""" Obsoluted
def order_rsp_check_func(rsp_data): #order_rsp is routing_key
    #print 'order_rsp_check_func() called'
    assert_is_instance(rsp_data, TkernelTas1_pb2.EntrustOrderRsp)
...
def main():
    ...
    tester = msgbus_perf_h.Tester(codec, display_interval=2)
    ...
    tester.add_driver('driver_order_req_1', bus_ip, bus_port, vhost, username, password, 'entry',
                      'order_req', data_list, 0.0002,
                      'busproxyorder_rsp1', 'order_rsp', 4)
    try:
        tester.start_test()
    finally:
        msgbus_perf_h.clear_msgbus_queue(bus_ip, bus_port, vhost, username, password,
